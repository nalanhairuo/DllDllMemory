...
static BOOL WINAPI dllmain_crt_dispatch(HINSTANCE const instance, DWORD const reason, LPVOID const reserved) throw()
{
    switch (reason)
    {
...
extern "C" BOOL WINAPI _CRT_INIT(HINSTANCE const instance, DWORD const reason, LPVOID const reserved)
{
    return dllmain_crt_dispatch(instance, reason, reserved);
}

static BOOL WINAPI dllmain_raw(HINSTANCE const instance, DWORD const reason, LPVOID const reserved) throw()
{
    if (!_pRawDllMain)
    {
        return TRUE;
...
static BOOL __cdecl dllmain_dispatch(HINSTANCE const instance, DWORD const reason, LPVOID const reserved) throw()
{
...
extern "C" __declspec(naked) __declspec(dllexport) BOOL WINAPI _DllMainCRTStartup(HINSTANCE const instance, DWORD const reason, LPVOID const reserved)
{
    ...
    if (dllmain_dispatch(instance, reason, reserved))
    {
        if (reason == DLL_PROCESS_ATTACH)
        {
            ...
            _asm push   DWORD PTR[esp - 0x0C]   //  instance 压栈
